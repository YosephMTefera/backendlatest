...
const ReplyOutgoingLetter = require("../../model/ReplyOutgoingLetters/ReplyOutgoingLetter");
const ForwardOutgoingLetter = require("../../model/ForwardOutgoingLetters/ForwardOutgoingLetter");
const ReplyOutgoingLetterHistory = require("../../model/ReplyOutgoingLetters/ReplyOutgoingLetterHistory");
...
const {
  appendReplyOutgoingLetterPrint,
} = require("../../middleware/replyOutgoingLtrPrt");
...
const replyOutgoingLetter = async (req, res) => {
  try {
    const expectedURLKey = process?.env?.GET_CRRPLYOUPTLTR_API;
    const actualAPIKey = req?.headers?.get_crrplyouptltr_api;
    if (actualAPIKey?.toString() === expectedURLKey?.toString()) {
      ...
      if (
        ...
        return res.status(StatusCodes.BAD_REQUEST).json({
          Message_en: "Please specify the outgoing letter you want to reply to",
          Message_am: "እባክዎን መልስ መስጠት የሚፈልጉትን ወጪ ደብዳቤ ያቅርቡ",
        });
      }

      const findOutgoingLetter = await OutgoingLetter.findOne({
        _id: outgoing_letter_id,
      });

      if (!findOutgoingLetter) {
        return res.status(StatusCodes.NOT_FOUND).json({
          Message_en:
            "The outgoing letter that you want to place your reply is not found",
          Message_am: "መልስዎን ማስቀመጥ የሚፈልጉበት ወጪ ደብዳቤ አልተገኘም",
        });
      }

      const findForwardOutgoingLetter = await ForwardOutgoingLetter.findOne({
        outgoing_letter_id: outgoing_letter_id,
      });

      if (!findForwardOutgoingLetter) {
        return res.status(StatusCodes.FORBIDDEN).json({
          Message_en:
            "Since the outgoing letter has not been forwarded yet, replies cannot be sent for this letter",
          Message_am: "ወጪ ደብዳቤው እስካሁን ስላልተላለፈ ለዚህ ደብዳቤ ምላሽ መላክ አይቻልም",
        });
      }

      const findReplyingPersonInForward =
        findForwardOutgoingLetter?.path?.filter(
          (item) => item?.to?.toString() === requesterId?.toString()
        );

      if (
        findReplyingPersonInForward?.length === 0 &&
        findOutgoingLetter?.createdBy?.toString() !== requesterId?.toString()
      ) {
        return res.status(StatusCodes.FORBIDDEN).json({
          Message_en:
            "You cannot reply to this outgoing letter as it has not been forwarded to you or created by you",
          Message_am:
            "ይህ ወጪ ደብዳቤ ለእርስዎ አልተላከም ወይም በእርስዎ አልተፈጠርም (not created) ፤ ስለዚህ ለዚህ ደብዳቤ መልስ መስጠት አይችሉም",
        });
      }
      ...
      const findReplyOutgoingLetter = await ReplyOutgoingLetter.findOne({
        outgoing_letter_id: outgoing_letter_id,
      });

      if (findReplyOutgoingLetter) {
        const findNormalForward = findReplyingPersonInForward?.find(
          (item) => item?.cc === "no"
        );

        const findSendingUserInReply = findReplyOutgoingLetter?.path?.filter(
          (item) => item?.to?.toString() === requesterId?.toString()
        );
        ...
        if (findSendingUserInReply?.length > 0) {
          ...
          if (
            !findNormal &&
            !findNormalForward &&
            findOutgoingLetter?.createdBy?.toString() !==
              requesterId?.toString()
          ) {
            return res.status(StatusCodes.FORBIDDEN).json({
              Message_en:
                "You cannot reply to this outgoing letter as it was only CC'd to you",
              Message_am: "ለእርስዎ CC ብቻ ስለነበር የተደረገው ለዚህ ወጪ ደብዳቤ መልስ መስጠት አይችሉም",
            });
          }
        }
      }
      ...
      form.parse(req, async (err, fields, files) => {
        ...
        const findCCForwardTo = findForwardOutgoingLetter?.path?.filter(
        ...
        if (
          ...
          findOutgoingLetter?.createdBy?.toString() !== to?.toString()
        ) {
          return res.status(StatusCodes.BAD_REQUEST).json({
            Message_en: `You cannot reply to ${findAcceptorUser?.firstname} ${findAcceptorUser?.middlename} ${findAcceptorUser?.lastname} as this outgoing letter was not forwarded to him/her`,
            Message_am: `ይህ ወጪ ደብዳቤ ወደ እሱ/ሷ ስላልተላከ ለ${findAcceptorUser?.firstname} ${findAcceptorUser?.middlename} ${findAcceptorUser?.lastname} ምላሽ መስጠት አይችሉም`,
          });
        }
        ...
        if (!findReplyOutgoingLetter) {
          ...
          const createReplyOutgoingLtr = await ReplyOutgoingLetter.create({
            outgoing_letter_id: outgoing_letter_id,
            forward_outgoing_letter_id: findForwardOutgoingLetter?._id,
            path,
          });
          ...
          try {
            await ReplyOutgoingLetterHistory.create({
              reply_outgoing_letter_id: createReplyOutgoingLtr?._id,
              updateHistory,
              history: createReplyOutgoingLtr?.toObject(),
            });
          } catch (error) {
            console.log(
              `Reply history for outgoing letter with ID (${findOutgoingLetter?._id}) is not created`
            );
          }
          ...
          const notificationMessage = {
            Message_en:
              cc === "no"
                ? `Replies for an outgoing is sent to you by ${findRequesterOfficeUser?.firstname} ${findRequesterOfficeUser?.middlename} ${findRequesterOfficeUser?.lastname}.`
                : `An outgoing letter is CC'd to you by ${findRequesterOfficeUser?.firstname} ${findRequesterOfficeUser?.middlename} ${findRequesterOfficeUser?.lastname}.`,
            Message_am:
              cc === "no"
                ? `ለወጪ ደብዳቤ ከ ${findRequesterOfficeUser?.firstname} ${findRequesterOfficeUser?.middlename} ${findRequesterOfficeUser?.lastname} ለእርስዎ መልስ ተልኮሎታል።`
                : `ለወጪ ደብዳቤ ከ ${findRequesterOfficeUser?.firstname} ${findRequesterOfficeUser?.middlename} ${findRequesterOfficeUser?.lastname} ለእርስዎ CC ተደርጓል።`,
          };

          await Notification.create({
            office_user: findAcceptorUser?._id,
            notifcation_type: "OutgoingLetter",
            document_id: findOutgoingLetter?._id,
          ...
          if (user) {
            io.to(user?.socketID).emit("outgoing_letter_reply_notification", {
              Message_en: `Replies for outgoing letter is sent to you.`,
              Message_am: `የወጪ ደብዳቤ ወደ እርስዎ መልስ ተልኳል።`,
            });
          }

          return res.status(StatusCodes.CREATED).json({
            Message_en: `Reply for outgoing letter is successfully sent to ${findAcceptorUser?.firstname} ${findAcceptorUser?.middlename} ${findAcceptorUser?.lastname}.`,
            Message_am: `ለወጪ ደብዳቤ ምላሽ በተሳካ ሁኔታ ወደ ${findAcceptorUser?.firstname} ${findAcceptorUser?.middlename} ${findAcceptorUser?.lastname} ተልኳል።`,
          });
        }

        if (findReplyOutgoingLetter) {
          const updateOutgoingReply =
            await ReplyOutgoingLetter.findOneAndUpdate(
              { _id: findReplyOutgoingLetter?._id },
              {
                $push: {
                  path: {
                    replied_date: new Date(),
          ...
          if (!updateOutgoingReply) {
            return res.status(StatusCodes.EXPECTATION_FAILED).json({
              Message_en: `The attempt to reply to an outgoing letter was not successful.`,
              Message_am: `ለወጪ ደብዳቤዉ መልስ ለመስጠት የተደረገው ሙከራ አልተሳካም።`,
            });
          }

          try {
            await ReplyOutgoingLetterHistory.findOneAndUpdate(
              { reply_outgoing_letter_id: findReplyOutgoingLetter?._id },
              {
                $push: {
                  updateHistory: {
                    updatedByOfficeUser: requesterId,
                  ...
                  history: updateOutgoingReply?.toObject(),
                },
              }
            );
          } catch (error) {
            console.log(
              `Reply history of outgoing letter with ID ${findOutgoingLetter?._id} is not updated successfully`
            );
          }
          ...
          const notificationMessage = {
            Message_en:
              cc === "no"
                ? `Replies for an outgoing letter is sent to you by ${findRequesterOfficeUser?.firstname} ${findRequesterOfficeUser?.middlename} ${findRequesterOfficeUser?.lastname}.`
                : `An outgoing letter is CC'd to you by ${findRequesterOfficeUser?.firstname} ${findRequesterOfficeUser?.middlename} ${findRequesterOfficeUser?.lastname}.`,
            Message_am:
              cc === "no"
                ? `ለወጪ ደብዳቤ ከ ${findRequesterOfficeUser?.firstname} ${findRequesterOfficeUser?.middlename} ${findRequesterOfficeUser?.lastname} ለእርስዎ መልስ ተልኮሎታል።`
                : `ወጪ ደብዳቤ ከ ${findRequesterOfficeUser?.firstname} ${findRequesterOfficeUser?.middlename} ${findRequesterOfficeUser?.lastname} ለእርስዎ CC ተደርጓል።`,
          };
...
const getRepliedOutgoingLetter = async (req, res) => {
  try {
    const expectedURLKey = process?.env?.GET_RPLIEDOUTPTLTR_API;
    const actualAPIKey = req?.headers?.get_rpliedoutptltr_api;
    if (actualAPIKey?.toString() === expectedURLKey?.toString()) {
      const requesterId = req?.user?.id;
      if (!requesterId || !mongoose.isValidObjectId(requesterId)) {
        return res.status(StatusCodes.UNAUTHORIZED).json({
      ...
      const repliedOutgoingLetters = await ReplyOutgoingLetter.find({
        "path.to": requesterId,
        "path.cc": "no",
      });

      if (!repliedOutgoingLetters) {
        return res.status(StatusCodes.NOT_FOUND).json({
          Message_en: "You have no replied outgoing letters",
          Message_am: "ምንም መልሶች የተሰጡባቸው ወጪ ደብዳቤዎች የሎዎትም",
        });
      }

      for (const replyPath of repliedOutgoingLetters) {
      ...
      const letterIds = replyList?.map(
        (rplyLtr) => rplyLtr?.outgoing_letter_id
      );
      ...
      const lstOfReplyOutgoingLtr = [];

      for (const items of findOutgoingLtrs) {
        const findReplyOutgoingLtr = await ReplyOutgoingLetter.findOne({
        ...
        if (findReplyOutgoingLtr) {
          caseFind = "yes";
        }

        const updatedItem = { ...items.toObject(), caseReplied: caseFind };

        lstOfReplyOutgoingLtr.push(updatedItem);
      }

      return res.status(StatusCodes.OK).json({
        outgoingLetters: lstOfReplyOutgoingLtr,
        totalOutgoingLtrs,
        currentPage: page,
...
const getRepliedOutgoingLetterCC = async (req, res) => {
  try {
    const expectedURLKey = process?.env?.GET_RPLEDOUTPTCCLTR_API;
    const actualAPIKey = req?.headers?.get_rpledoutptccltr_api;
    if (actualAPIKey?.toString() === expectedURLKey?.toString()) {
      const requesterId = req?.user?.id;
      if (!requesterId || !mongoose.isValidObjectId(requesterId)) {
        return res.status(StatusCodes.UNAUTHORIZED).json({
      ...
      if (!repliedOutgoingLetters) {
        return res.status(StatusCodes.NOT_FOUND).json({
          Message_en: "You have no CC'd outgoing letters",
          Message_am: "ምንም CC የተደረጉ ወጪ ደብዳቤዎች የሎዎትም",
        });
      }
...
const getReplyOutgoingLetterPath = async (req, res) => {
  try {
    const expectedURLKey = process?.env?.GET_RPLDOUTPTLTRPATH_API;
    const actualAPIKey = req?.headers?.get_rpldoutptltrpath_api;
    if (actualAPIKey?.toString() === expectedURLKey?.toString()) {
      const requesterId = req?.user?.id;
      if (!requesterId || !mongoose.isValidObjectId(requesterId)) {
        return res.status(StatusCodes.UNAUTHORIZED).json({
      ...
      if (!findOutgoingLtrs || !findReplyOutgoingLtr) {
        return res.status(StatusCodes.NOT_FOUND).json({
      ...
      const replyLetters = findReplyOutgoingLtr?.path;

      return res.status(StatusCodes.OK).json({
        repliedDocs: replyLetters,
        replyId: findReplyOutgoingLtr?._id,
      });
    } else {
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json();
    }
...
const printReplyOutgoingLetter = async (req, res) => {
  try {
    const expectedURLKey = process?.env?.GET_RPLDOUTPTLTRPRT_API;
    const actualAPIKey = req?.headers?.get_rpldoutptltrprt_api;
    if (actualAPIKey?.toString() === expectedURLKey?.toString()) {
      const requesterId = req?.user?.id;
      if (!requesterId || !mongoose.isValidObjectId(requesterId)) {
        return res.status(StatusCodes.UNAUTHORIZED).json({
      ...
      const findOutgoingLetter = await OutgoingLetter.findOne({
        _id: findReplyOutgoingLetter?.outgoing_letter_id,
      });

      if (!findOutgoingLetter || !findReplyOutgoingLetter) {
        return res.status(StatusCodes.NOT_FOUND).json({
          Message_en: "The letter and its replies were not found",
      ...
      const replyToPrint = findReplyOutgoingLetter?.path?.find(
      ...
      if (!findSenderUser) {
        return res.status(StatusCodes.NOT_FOUND).json({
          Message_en: `The person who replied to this outgoing letter is not found among the office administrators.`,
          Message_am: `ለዚህ ወጪ ደብዳቤ ምላሽ የሰጡት ተጠቃሚ በቢሮ አስተዳዳሪዎች መካከል አልተገኘም።`,
        });
      }
      ...
      if (!findRecieverUser) {
        return res.status(StatusCodes.NOT_FOUND).json({
          Message_en: `The person who received the answer to this outgoing letter is not found among the office administrators.`,
          Message_am: `ይህንን ወጪ ደብዳቤ መልስ የተቀበለው ተጠቃሚ ከቢሮ አስተዳዳሪዎች መካከል አልተገኘም።`,
        });
      }

      const sent_date = caseSubDate(replyToPrint?.replied_date);
      const case_num = findOutgoingLetter?.outgoing_letter_number
        ? findOutgoingLetter?.outgoing_letter_number
        : "ቁጥር አልተሰጠዉም";
      const sent_from =
      ...
      const outputPath = join(
        "./",
        "Media",
        "ReplyOutgoingLetterPrint",
        uniqueSuffix + "-replyoutgoingletter.pdf"
      );
      ...
      try {
        await appendReplyOutgoingLetterPrint(inputPath, text, outputPath);
...
module.exports = {
  replyOutgoingLetter,
  getRepliedOutgoingLetter,
  getRepliedOutgoingLetterCC,
  getReplyOutgoingLetterPath,
  printReplyOutgoingLetter,
};
